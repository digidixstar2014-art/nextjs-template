...
// Minimal payload item shape used by Recharts' tooltip/legend items
...
interface TooltipOwnProps extends React.ComponentProps<"div"> {
  active?: boolean
...
const ChartTooltipContent = React.forwardRef<HTMLDivElement, TooltipOwnProps>((props, ref) => {
  const {
    active,
    payload,
    className,
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const it = item as RechartsPayloadItem
    const key = `${labelKey || it.dataKey || it.name || "value"}`
    ...
    if (labelFormatter) {
      return (
        <div className={cn("font-medium", labelClassName)}>
          {labelFormatter(value, payload as RechartsPayloadItem[])}
        </div>
      )
    }
  ...
  const nestLabel = (payload as RechartsPayloadItem[]).length === 1 && indicator !== "dot"
              ...
              {formatter && it?.value !== undefined && it.name ? (
                ...
                <>
                  ...
                  <div className={cn("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center") }>
                    <div className="grid gap-1.5">
                      {nestLabel ? tooltipLabel : null}
                      <span className="text-muted-foreground">{itemConfig?.label || it.name}</span>
                    </div>
                    {it.value && <span className="font-mono font-medium tabular-nums text-foreground">{it.value.toLocaleString()}</span>}
                  </div>
                </>
              )}
            </div>
...
interface LegendOwnProps extends React.ComponentProps<"div"> {
...
const ChartLegendContent = React.forwardRef<HTMLDivElement, LegendOwnProps>((props, ref) => {
  ...
  return (
    ...
    >
      {payload?.map((item: RechartsPayloadItem) => {
